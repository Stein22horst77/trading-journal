...
async function init() {
  ...
  await pool.query(`ALTER TABLE trades ADD COLUMN IF NOT EXISTS account TEXT NOT NULL DEFAULT 'Main'`);
  for (const c of ["take_profit","stop_loss","max_price","min_price"])
    await pool.query(`ALTER TABLE trades ADD COLUMN IF NOT EXISTS ${c} NUMERIC`);
...
const FIELDS = ["symbol","account","asset_type","style","direction","entry_date","exit_date",
  "entry_price","exit_price","quantity","fees","risk","take_profit","stop_loss","max_price","min_price","setup","notes","starred","chart_image"];

function values(b) {
  return [
    String(b.symbol || "").trim().toUpperCase(),
    (b.account || "Main").trim() || "Main",
    ...
    b.take_profit ?? null, b.stop_loss ?? null, b.max_price ?? null, b.min_price ?? null,
...
app.get("/api/trades", async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, symbol, account, asset_type, style, direction, entry_date, exit_date,
              entry_price, exit_price, quantity, fees, risk, take_profit, stop_loss, max_price, min_price, setup, notes, starred,
